    ...
    <div className="space-y-3">
      ...
      {qrCodeUrl && (
        <div className="flex justify-center rounded-lg border bg-white p-4">
          <img src={qrCodeUrl || "/placeholder.svg"} alt="Room QR Code" className="h-40 w-40" />
        </div>
      )}
      ...
      <div className="flex gap-2">
        ...
        <Button variant="outline" className="flex-1 bg-transparent" onClick={handleDownloadQR}>
          <Download className="mr-2 h-4 w-4" />
          下载
        </Button>
      </div>

      {shareLink && <p className="text-xs text-muted-foreground break-all">{shareLink}</p>}
    </div>
  )
}
