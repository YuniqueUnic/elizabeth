...
import { Copy, Download, Save, Trash2, HelpCircle, Settings } from "lucide-react"
import { useQuery } from "@tanstack/react-query"
import { getRoomDetails, getMessages } from "@/api/roomService"
...
export function TopBar() {
  ...
  const includeMetadataInExport = useAppStore((state) => state.includeMetadataInExport)
    ...
    const selectedMessagesList = messages
      ...
      .map((m, index) => {
        if (includeMetadataInExport) {
          const messageNumber = messages.findIndex((msg) => msg.id === m.id) + 1
      ...
      <div className="flex items-center gap-3">
        ...
        {roomDetails && (
          <div className="ml-4 text-sm text-muted-foreground">
            房间占用：{roomDetails.currentSize.toFixed(1)} / {roomDetails.maxSize} MB
          </div>
        )}
      </div>
