...
消息现在支持多选、全选、反转选择等操作，用户可以：

- 单击消息左侧的复选框选中单条消息
- 使用工具栏的全选/取消全选/反选按钮批量操作
- 通过顶栏的复制按钮将选中消息复制到剪贴板
- 通过顶栏的下载按钮将选中消息导出为 Markdown 文件
- 在设置中配置是否在导出时包含元数据（时间戳、消息编号）

**涉及组件**:

- `MessageBubble`: 添加了复选框支持
- `MessageList`: 添加了选择工具栏
- `TopBar`: 添加了复制和导出功能
...
- `RightSidebar`: 文件选择工具栏
- `FileListView`: 文件列表视图

#### 3. 移动端响应式设计

在移动端（屏幕宽度 < 768px），应用自动切换为 Tab 布局：

- **设置 Tab**: 房间设置、权限、容量、分享
- **聊天 Tab**: 消息列表和输入框（默认）
- **文件 Tab**: 文件列表和上传区
...
- `MobileLayout`: 新建的移动端布局组件
- `LeftSidebar`, `RightSidebar`: 支持移动端全宽显示
- `HomePage`: 根据屏幕宽度自动切换布局

#### 4. Markdown 编辑器升级

使用 `@uiw/react-md-editor` 替换了原有的编辑器：
...
- `EnhancedMarkdownEditor`: 新的 Markdown 编辑器组件
- `MessageInput`: 使用新编辑器
...
- 支持多种编程语言
- 主题跟随系统设置（github-dark / github-light）
- 显示语言标识
- 一键复制代码

**涉及组件**:

- `CodeHighlighter`: 新的代码高亮组件
- `MarkdownRenderer`: 使用新的代码高亮

### 技术栈更新

新增依赖：

- `@uiw/react-md-editor@^4.0.8`: Markdown 编辑器
- `shiki@latest`: 代码语法高亮
...
1. 在消息列表中，点击消息左侧的复选框选中消息
2. 使用顶部工具栏的全选/反选按钮批量操作
3. 选中消息后，点击顶栏的复制或下载按钮
4. 在设置中配置是否包含元数据

#### 如何使用移动端布局

移动端布局会自动激活（屏幕宽度 < 768px）：
...
代码块会自动应用语法高亮，并显示语言标识和复制按钮。

### 已知问题

#### Next.js 16 字体加载问题

当前项目使用 Next.js 16，存在字体加载相关的构建错误。这是 Next.js
的已知问题，不影响开发环境的运行。

**临时解决方案**:

- 使用 `npm run dev` 启动开发服务器正常工作
- 生产构建可能需要降级到 Next.js 15 或等待 Next.js 16 修复
...
- `hooks/use-mobile.ts`: 定义了移动端断点（768px）
- `components/layout/mobile-layout.tsx`: 移动端布局
- `app/page.tsx`: 布局切换逻辑
...
- 修改 `components/chat/enhanced-markdown-editor.tsx`
- 参考 [@uiw/react-md-editor 文档](https://github.com/uiwjs/react-md-editor)
- 确保新功能在移动端也能正常使用

#### 添加新的语言高亮

Shiki 支持多种语言，无需额外配置。如需自定义：

- 修改 `components/chat/code-highlighter.tsx`
- 参考 [Shiki 文档](https://shiki.style)
- 可以添加自定义主题和语言

---

**文档版本**: v2.0 **最后更新**: 2025-10-26 **维护者**: Elizabeth 开发团队
