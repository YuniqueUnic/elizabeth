...
export default defineConfig<SerenityFixtures, SerenityWorkerFixtures>({
  ...
  webServer: {
    command:
      ...
      // e2e fixture 依赖平台管理员账号补签身份码（如二次加入同一房间），
      // 与 CallElizabethApi.ability 的 ADMIN_BOOTSTRAP_PASSWORD 默认值保持一致。
      // 账号仅在首次启动时 bootstrap；测试 DB 每次运行独立，因此必然创建。
      `ELIZABETH_ADMIN_USERNAME='${process.env.ELIZABETH_ADMIN_USERNAME ?? "admin"}' ` +
      `ELIZABETH_ADMIN_PASSWORD='${process.env.ELIZABETH_ADMIN_PASSWORD ?? "elizabeth-test-admin"}' ` +
