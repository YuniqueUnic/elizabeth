...
1. 前端的创建房间的 Option 密码却少一个显示/隐藏密码功能
   1. 如果显示了密码，那么就只有一个密码框
   2. 如果没有显示密码，那么就两个密码框 (两个用于确保用户输入的密码一致)
   3. 现在用户加入房间时需要输入两次密码... 这个行为是不对...
2. 后端 AppConfig.LoggingConfig.level 解析存在问题，始终为 "off",
   需要找到原因，并且修复
3. 通过 https://box.yunique.top 访问前端时，创建房间时不能成功
   (现在能成功了，但是还是存在一些问题，比如过一段时间就会报错，信息不可访问相关的错误，我怀疑还是
   https 和 http 混合访问带来的问题.)
   1. 是由于前端的请求来自 https, 而前端和后端都是 http 交互。
   2. 前端和后端是通过 docker-compose 启动的一组容器。
   3. 需要找到解决办法

由于会有 https mixcontent 的问题。请你思考一个很完美的解决方案来解决它。

所以请你仔细思考一下，根据项目代码，来选择一个最佳的方式。重构相关代码，优化逻辑，改进代码。

## 是记得保持 KISS, LISP, DRY, SOLID 等规则，保持代码函数化，模块化，组件化，保持可组合性，保持简单。禁止过于冗余，并且没有用的代码请你直接删除。避免导致代码的无效冗余。
