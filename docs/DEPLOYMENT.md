...
同一房间内重复上传相同内容时，物理存储只保留一份（按 SHA-256 内容寻址），
内容记录通过引用计数共享对象；删除到引用归零才物理清理。客户端在上传 prepare
阶段携带整文件 SHA-256（`file_hash`）即可对已存在内容"秒传"——
跳过传输直接建内容记录。

- 去重范围默认 **per-room**：跨房间全局去重会构成"文件是否已存在"的
  探测通道（知道哈希即可试探其他房间是否有该文件），因此默认关闭；
  全局去重留作平台级显式开关，开启前请评估该隐私代价。
- 客户端谎报哈希或大小不符时不会命中，自动回退正常上传。
- presigned 直传与存量历史内容不参与去重（无服务端哈希），删除按 旧路径处理。
