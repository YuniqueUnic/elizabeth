# Elizabeth 项目前端路由和页面结构详细文档

## 模块概述

路由和页面结构模块是 Elizabeth 项目的前端导航核心，基于 Next.js 14 App Router
实现。该模块负责应用的页面路由管理、导航控制、数据获取策略和响应式设计，为用户提供了流畅的单页应用体验。

## 功能描述

路由和页面结构模块主要负责：

- 提供基于文件系统的路由管理
- 支持动态路由和静态路由生成
- 实现页面间的导航和状态管理
- 提供服务端渲染（SSR）和客户端渲染（CSR）的混合模式
- 支持响应式设计和移动端优化
- 集成 SEO 优化和可访问性功能
- 提供路由守卫和权限控制机制

## 页面清单

### 1. 主页面 (page.tsx)

**文件位置**: `web/app/page.tsx`

**功能描述**: 应用入口页面，提供房间创建和加入功能

**核心特性**:

- **模式切换**: 支持主页、创建房间、加入房间三种模式
- **表单处理**: 房间创建和加入表单，支持密码保护
- **错误处理**: 完善的错误捕获和显示机制
- **主题切换**: 集成主题切换器
- **路由导航**: 使用 Next.js App Router 进行页面跳转

### 2. 动态房间页面 ([roomName]/page.tsx)

**文件位置**: `web/app/[roomName]/page.tsx`

**功能描述**: 房间的主要协作界面，包含聊天、文件管理和设置功能

**核心特性**:

- **参数获取**: 使用 `useParams` 获取房间名称
- **认证流程**: 多层认证验证（令牌验证 → 刷新令牌 → 密码验证）
- **权限控制**: 基于令牌的房间访问控制
- **响应式布局**: 桌面端和移动端不同的布局组件
- **数据获取**: 使用 React Query 进行房间数据获取和缓存管理

### 3. 根布局 (layout.tsx)

**文件位置**: `web/app/layout.tsx`

**功能描述**: 全局布局包装器，提供主题、字体管理和通知系统

**核心特性**:

- **元数据配置**: 设置页面标题、描述和生成器信息
- **样式集成**: 引入全局样式和第三方样式（Heti）
- **组件包装**: 使用 Provider 模式包装应用
- **功能集成**: 集成字体管理器和通知系统

## 路由设计和导航

### 导航机制

#### 程序化导航

```typescript
import { useRouter } from "next/navigation";

export default function NavigationExample() {
  const router = useRouter();

  const navigateToHome = () => router.push("/");
  const navigateToRoom = (roomName: string) => router.push(`/room/${roomName}`);
  const navigateBack = () => router.back();

  return (
    <nav>
      <button onClick={navigateToHome}>首页</button>
      <button onClick={() => navigateToRoom("demo-room")}>进入房间</button>
      <button onClick={navigateBack}>返回</button>
    </nav>
  );
}
```

#### 参数传递

```typescript
// 查询参数传递
const SearchResults = () => {
  const router = useRouter();
  const searchParams = useSearchParams();

  const handleSearch = (query: string) => {
    router.push(`/search?q=${encodeURIComponent(query)}`);
  };

  return (
    <div>
      <input onChange={(e) => setSearch(e.target.value)} />
      <button onClick={() => handleSearch(searchParams.get("q") || "")}>
        搜索
      </button>
    </div>
  );
};
```

### 路由守卫和权限控制

```typescript
// 路由守卫示例
import { useEffect } from "react";
import { useRouter } from "next/navigation";
import { getCurrentRoomToken } from "@/lib/store";

export default function ProtectedRoute({ children, requiredPermission }) {
  const router = useRouter();
  const token = getCurrentRoomToken();

  useEffect(() => {
    if (!token || !hasPermission(token, requiredPermission)) {
      router.push("/login");
    }
  }, [token, requiredPermission]);

  return <>{children}</>;
}
```

## 页面数据获取

### 服务端渲染（SSR）和数据获取策略

#### 混合渲染策略

- **主页面**: 客户端渲染（CSR），无服务端数据需求
- **房间页面**: 混合模式，初始加载使用 SSR，后续操作使用 CSR

#### 数据获取时机

- **页面初始化**: 在 `useEffect` 中获取房间数据
- **令牌验证**: 服务端验证令牌有效性
- **数据缓存**: 本地存储令牌信息，减少网络请求

### 客户端渲染（CSR）和数据水合

#### 客户端状态

- **状态管理**: 使用 React Hooks 管理组件状态
- **数据水合**: 服务端渲染初始状态，客户端接管后续交互
- **离线支持**: 本地存储支持离线操作

### 静态生成（SSG）和缓存策略

#### 静态生成

- **主页面**: 适合 SSG，内容相对静态
- **房间页面**: 动态内容，不适合 SSG，使用 SSR + CSR

#### 缓存策略

- **令牌缓存**: 本地存储令牌，设置过期时间
- **API 缓存**: 合理的 API 请求缓存

## 响应式页面设计

### 移动端和桌面端适配

#### 响应式断点

- **移动端**: 使用 Tabs 组件的垂直布局
- **桌面端**: 使用三栏布局（左、中、右侧边栏）
- **检测机制**: 自定义 `useIsMobile` Hook

#### 页面组件的条件渲染

- **设备检测**: 基于 `useIsMobile` 条件渲染不同组件
- **功能降级**: 移动端简化部分功能
- **性能优化**: 避免不必要的组件渲染

### 移动端布局

#### 组件结构

- **布局**: 底部标签栏 + 主内容区域
- **导航模式**: 标签式切换（设置、聊天、文件）
- **空间利用**: 优化的移动端空间利用

### 桌面端布局

#### 组件结构

- **布局**: 左侧边栏 + 中间内容区 + 右侧边栏
- **功能分布**: 每个区域专注特定功能
- **交互优化**: 针对鼠标操作优化

## 性能优化和最佳实践

### 代码分割和懒加载策略

#### 代码分割

- **自动分割**: Next.js App Router 自动代码分割
- **组件懒加载**: 使用 `next/dynamic` 进行动态导入
- **路由级分割**: 每个页面自动分割

#### 页面预加载和缓存机制

- **预加载策略**: 重要链接的预加载提示
- **数据预加载**: 房间数据的预获取
- **缓存策略**: HTTP 缓存和本地缓存结合

### SEO 和可访问性优化

#### SEO 优化

- **元数据**: 动态生成页面元数据
- **语义化 HTML**: 使用正确的 HTML5 语义标签
- **结构化数据**: 支持搜索引擎理解

#### 可访问性

- **键盘导航**: 支持键盘操作
- **屏幕阅读器**: 适当的 ARIA 标签
- **焦点管理**: 合理的焦点管理

## 技术栈和依赖

### 核心技术栈

- **前端框架**: Next.js 14 (App Router)
- **UI 库**: React + TypeScript + Tailwind CSS + shadcn/ui
- **状态管理**: Zustand
- **HTTP 客户端**: 自定义 API 工具
- **样式方案**: Tailwind CSS + CSS-in-JS
- **构建工具**: Next.js 内置构建系统

### 主要依赖包

```json
{
  "dependencies": {
    "next": "14.2.5",
    "react": "^18.3.1",
    "@radix-ui/react-*": "^1.0.3",
    "class-variance-authority": "^0.7.0",
    "clsx": "^2.1.0",
    "tailwind-merge": "^2.3.0",
    "lucide-react": "^0.394.0",
    "zustand": "^4.5.2",
    "joi": "^17.12.0"
  },
  "devDependencies": {
    "@types/node": "^20.12.8",
    "typescript": "^5.6.3",
    "eslint": "^8.57.0",
    "@next/eslint-config-next": "^14.2.5"
  }
}
```

## 配置和构建设置

### Next.js 配置

```javascript
/** @type {import('next').NextConfig} */
const nextConfig = {
  typescript: {
    ignoreBuildErrors: true,
  },
  images: {
    unoptimized: true,
  },
  allowedDevOrigins: ["local-origin.dev", "*.local-origin.dev"],
};
```

## 路由图（文字描述）

### 应用整体路由流程

```
用户访问应用
    ↓
主页面 (/)
    ├── 创建房间模式
    │   ├── 填写房间信息
    │   ├── 提交创建请求
    │   └── 获取令牌
    │       ↓
    │       导航到房间页面
    ├── 加入房间模式
    │   ├── 填写房间名称
    │   └── 直接导航到房间页面
    │           ↓
    │           房间页面 (/[roomName])
    │           ├── 检查本地令牌
    │           ├── 无令牌：获取房间详情
    │           │   ├── 公开房间：直接进入
    │           │   └── 私有房间：显示密码输入
    │           ├── 有令牌：验证令牌有效性
    │           │   ├── 有效：直接进入房间
    │           │   └── 无效：刷新令牌或重新输入密码
    │           │           └── 显示房间主界面
    │                   ├── 左侧边栏（设置）
    │                   ├── 中间内容区（聊天）
    │                   └── 右侧边栏（文件）
    │                       ↓
    │                       各种房间操作（上传、下载、权限管理）
    │                       ↓
    │                       导航到其他页面或退出
    │                           ↓
    │                           返回主页面
```

### 数据流向图

```
用户交互
    ↓
页面组件 (React Components)
    ↓
状态管理 (Zustand Store)
    ↓
API 服务层 (Services)
    ↓
HTTP 客户端 (API Utils)
    ↓
后端 API (Rust Server)
```

## 架构优势

### 模块化设计

- 清晰的职责分离
- 可维护的代码组织
- 可扩展的组件设计

### 性能特点

- 混合渲染：SSR + CSR 的最佳实践
- 智能缓存：多层缓存策略
- 响应式设计：完善的移动端支持

### 安全特点

- 令牌管理：完善的认证和授权机制
- 权限控制：细粒度的房间权限管理
- 数据保护：客户端和服务端双重验证

该路由和页面结构模块为 Elizabeth
项目提供了现代化、高效、用户友好的导航解决方案，具备良好的可维护性和扩展性。
