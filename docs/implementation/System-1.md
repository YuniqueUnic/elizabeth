...
1. **实时过期检查**：所有房间相关操作都会检查房间是否过期
   - 使用 [`Room::is_expired()`](crates/board/src/models/room/mod.rs:75)
     检查过期状态
   - 过期房间在所有操作中都被视为不存在

2. **全局过期保护**：以下操作都会检查房间过期状态
   - 房间查找、删除、权限更新
   - 令牌签发、验证、撤销
   - 内容上传、下载、删除

3. **统一错误处理**：过期房间返回适当的 HTTP 错误码
   - 新用户访问：返回 403 Forbidden
   - 已进入用户操作：返回 401 Unauthorized
   - 房间删除操作：返回 410 Gone
...
5. **过期房间隔离**：过期房间在清理前已无法进行任何操作，确保数据安全
